...
export const Navbar = () => {
  const { weeks, getweekbybutton, moduleWeek, setWeeks } =
...
// console.log("weeks: before ", weeks);

    // setWeeks((prevWeeks) => prevWeeks.filter((wee) => wee.week_number !== moduleWeek.week_number));
    // console.log("weeks: after", weeks);

     axios.post(`/week/${moduleWeek.week_number}/deleteWeek`)
    ...
    <>
      <nav className="">
        <div className="flex p-3 rounded-3xl bg-white justify-between flex-col md:flex-row">
          ...
          <div className="flex items-center gap-4 justify-end mr-8 relative">
            <button
              className="bg-gray-300 text-gray-800 rounded-3xl flex gap-2 p-4 text-sm md:text-base hover:bg-gray-400"
              onClick={deleteWeekfun}
            >
              Delete Current Week
            </button>
            <button
